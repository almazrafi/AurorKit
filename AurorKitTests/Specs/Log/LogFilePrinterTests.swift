        ...
        describe(".init(encoding:fileHeader:fileName:)") {
            context("when the header is not empty") {
                ...
                it("should rewrite the content with the header") {
                    let firstPrinter = LogFilePrinter(
                        encoding: .utf8,
                        fileHeader: "First Tests Log",
                        fileName: "Tests.log"
                    )

                    let secondPrinter = LogFilePrinter(
                        encoding: .utf8,
                ...
                it("should rewrite the file with the header") {
                    ...
                    expect {
                        printer.flatMap { try? String(contentsOfFile: $0.filePath, encoding: .utf8) }
                    }.to(equal("Tests Log\n"))
                }
            }
        ...
        describe(".print(:)") {
            context("when the header is not empty") {
                ...
                it("should write the line to the file") {
                    ...
                    expect {
                        try? String(contentsOfFile: printer.filePath, encoding: .utf8)
                    }.to(equal("Tests Log\nSomething happened\n"))
                }

                it("should write the header and the line if even the file does not exist") {
                    try? FileManager.default.removeItem(atPath: printer.filePath)
